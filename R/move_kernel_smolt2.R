...
move_kernel_smolt2 <- function(data, xy = NULL, mpar, i, s, ts) {
      switch(mpar$scenario,
             sobi = {
                ...
                rho <- tanh(sqrt(delta[1]^2 + delta[2]^2) * mpar$pars$r + 0.2) - tanh(0.2)
  ...
  if(new.xy[1] > 6918.75 & new.xy[2] > 1468 & new.xy[2] < 1470.25) {
    ## move westward, parallel to land
    mu <- as.numeric(dir2l - 0.5 * pi)
  }

  ## movement to avoid central island
  if(new.xy[1] > 6916.25 & new.xy[2] > 1471 & new.xy[2] < 1473) {
    ## move eastward, parallel to land
    mu <- as.numeric(dir2l + 0.5 * pi)
  }

  if(new.xy[1] <= 6916.25 & new.xy[2] > 1472.25 & new.xy[2] < 1476.4) {
    ## move westward, parallel to island
    mu <- as.numeric(dir2l - 0.5 * pi)
  }

  ## movement to avoid western island
  if(new.xy[1] > 6912.5 & new.xy[1] < 6914.8 & new.xy[2] > 1477.5) {
  ...
  new.xy <- cbind(new.xy[1] + sin(mu) * s, new.xy[2] + cos(mu) * s)
